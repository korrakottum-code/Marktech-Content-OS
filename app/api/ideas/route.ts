...
type Slot = { slotId: string; mechanism: string; mechanismId: string; category: ContentCategory; funnel: string; product?: string; angle?: string; audienceContext?: string };
...
export async function POST(request: Request) {
  ...
  const products = (focusedProduct ? [focusedProduct] : input.briefs.map((brief) => brief.product?.trim()).filter((product): product is string => Boolean(product && product !== "ไม่ระบุโปรดักต์")));
  const productPool = products.length ? Array.from(new Set(products)) : ["ไม่ระบุโปรดักต์"];
  const slots = (buildMechanismSlots(target, planningGoal, allowedCategories, hasVerifiedOffer) as Slot[])
    .map((slot, index) => ({ ...slot, product: productPool[index % productPool.length] }));
  ...
  const brief = {
    ...
    focusedAddition: input.focusBrief ? { product: input.focusBrief.product ?? "", goal: input.focusBrief.goal ?? "", painFocus: input.focusBrief.painFocus ?? "", customNeed: input.focusBrief.customNeed ?? "" } : null,
  };
...
ถ้า brief มี painFocus ให้ถือว่าเป็น “แรงตึงของการตัดสินใจ” หลัก: อย่างน้อย 70% ของ slot ต้องทำให้ pain/situation นี้เฉพาะเจาะจงขึ้น ไม่ใช่เปลี่ยนกลับเป็นคำกว้างอย่าง “คุ้มค่า” หรือ “ทักเลย”
...
ค่า product ของไอเดียต้องตรงกับ product ที่ล็อกไว้ใน slot เท่านั้น ห้ามใช้ชื่อคลินิก ชื่อแบรนด์ หรือคำว่า “บริการ” แทนชื่อโปรดักต์ เว้นแต่ slot.product เป็น “ไม่ระบุโปรดักต์”
...
เมื่อมี painFocus title หรือ hook ของ Offer ต้องเริ่มจากสถานการณ์/ข้อกังวลที่ระบุ แล้วใช้ราคาเป็นคำตอบหรือหลักฐานภายหลัง ห้ามใช้ราคาลอย ๆ เป็นแกนเดียวของทุกชิ้น
...
`;
    ...
    const generateForSlots = async (batch: Slot[], avoidText = existingText || "ไม่มี") => {
      ...
      return cleanIdeas(result.ideas).map((idea, index) => {
        ...
        return slot ? { ...idea, slotId: slot.slotId, product: slot.product || idea.product, mechanism: slot.mechanism, funnel: slot.funnel, angle: slot.angle, category: slot.category } : idea;
      });
    };
