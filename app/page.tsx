  ...
  return <main className="planner-shell" id="top">
    ...
    <section className="flow-card" id="brief">
      ...
      {briefSummary && <section className="brief-review" aria-label="ตรวจโจทย์ก่อนสร้างไอเดีย">
        <div className="brief-review-heading"><div><p className="eyebrow">หยุดตรวจ 1 ครั้งก่อนสร้าง</p><h3>ตรวจโจทย์และภาพอ้างอิง</h3><p>ตอนนี้ AI แค่อ่านและแยกโจทย์เท่านั้น — ยังไม่ได้สร้างไอเดียแม้แต่ชิ้นเดียว</p></div><div className="brief-review-actions"><button className="text-button" type="button" onClick={() => setAdvancedBriefOpen(true)}>แก้รายละเอียด</button><button className="button button-primary" type="button" onClick={generateIdeas} disabled={isGenerating}>{isGenerating ? `AI กำลังคิด ${ideaPoolSize} ทางเลือก…` : `ยืนยัน · สร้าง ${ideaPoolSize} ไอเดีย →`}</button></div></div>
        <div className="brief-counts"><span>AI แยกได้ <strong>{reviewedBriefs.length} เรื่อง</strong> จากโจทย์</span><strong>เมื่อกดยืนยัน ระบบจะสร้าง <b>{ideaPoolSize} ไอเดีย</b></strong></div>
